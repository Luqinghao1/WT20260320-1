...
#include <boost/math/quadrature/gauss_kronrod.hpp> // [新增]: 引入 Boost Gauss-Kronrod 积分器
#include <cmath>
#include <algorithm>
#include <numeric>
#include <QDebug>
...
ModelSolver01_06::ModelSolver01_06(ModelType type)
    ...
    precomputeStehfestCoeffs(8); // [修改点]: 预计算默认 8 阶的 Stehfest 反演系数，压制高阶震荡
}
...
ModelCurveData ModelSolver01_06::calculateTheoreticalCurve(const QMap<QString, double>& params, const QVector<double>& providedTime)
{
    ...
    int N = (int)calcParams.value("N", 8); // [修改点]: 默认 Stehfest 阶数降为 8
                                           ...
                                           QVector<double>& outPD, QVector<double>& outDeriv)
{
    ...
    int N = (int)params.value("N", 8); // [修改点]: Stehfest 降维至 8
    ...
    for (int i = 0; i < n_fracs; ++i) {
        for (int j = 0; j < n_fracs; ++j) {
            ...
            if (i == j) {
                ...
                // [修改点]：使用 Boost.Math Gauss-Kronrod 积分器替代自定义自适应积分
                ...
                // [修改点]：使用 Boost.Math Gauss-Kronrod 积分器
                unsigned max_depth = 15;
                double tol = 1e-6;
                double error_est;
